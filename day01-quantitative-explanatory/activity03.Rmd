...
**-   int_mod has an R^2 of .73 and adj R^2 of .73 while mlr_mod has an R^2 of .72 and adj R^2 of .72. These value are all very similar, and have very small p-values. These stats tell us how much variation is accounted for by each model and then accounts for simplicity - adjusted R^2 will go down as you add variables that don't add much explanatory power to your model.**

NOTE: we could also break into test and train and assess fit on unseen test data.
...
**-   The F-test statistic for this model is testing how likely it is that we'd have our results given a flat slope for all model coefficients. The p-value calcultes the likelihood, given assumptions about the data's distribution.**
...
**-   The p-value suggests that we have reason to reject the null for that t-test statistic. This is reasonable evidence (.00126) that this term has an effect (non-zero slope) in the model with others held constant and in the model.**
...
```{r test-non-int}
#rerun same code just cuz
tidy(int_mod)
```
**-   We also have good evidence for these being significant at the .05 significance level, they're .00117 and .000245 for east_atlanticYes and pf_expression_control respectively. **
